...
jQuery(document).ready(function() {
    ...
    if( jQuery('#photo3').is(':visible') ) {    	
       createUploader('photo3','photo3');
    }     
	...
	$( document ).on( "click", ".remove-food-image", function(){
		var a=confirm(js_lang.trans_4);
		if(a)
		{
			remove_food_image();
...
function photo3(data)
{
	var img='';	
	$(".preview3").show();
	img+="<img src=\""+upload_url+"/"+data.details.file+"\" alt=\"\" title=\"\" class=\"uk-thumbnail uk-thumbnail-mini\" >";
	img+="<input type=\"hidden\" name=\"photo3\" value=\""+data.details.file+"\" >";
	img+="<p><a href=\"javascript:rm_preview3();\">"+js_lang.removeFeatureImage+"</a></p>";
	$(".image_preview3").html(img);
	$(".image_preview3").show();
}
...
function remove_food_image()
{
	var params="action=removeFoodimage";
	busy(true);
    $.ajax({    
    type: "POST",
    url: ajax_url,
