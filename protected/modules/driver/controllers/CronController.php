					...
					if (is_array($time_diff) && count($time_diff)>=1){
					   if ( $time_diff['hours']>0 || $time_diff['minutes']>=$request_expire){					   	    
					   	  if ( $driver=Driver::getUnAssignedDriver($task_id)){					   	      
					   	      ...
					   	      $params_driver=array('status'=>'process','date_process'=>date('Y-m-d H:i:s'));
					   	      dump($params_driver);
					   	  ...
					   	  $params['assign_started']=date('Y-m-d H:i:s');
					   	  dump($params);
			...
			foreach ($res as $val) {
				...
				if ( $res2=self::$db->rst($stmt2)){
					...
					self::$db->updateData("{{driver_bulk_push}}",
					   array('status'=>"process",'date_process'=>date('Y-m-d H:i:s'))
					   ,'bulk_id',$bulk_id);
				}
			}
