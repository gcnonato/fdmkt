...
{	
	...
	{				
		...
		if(!empty($cart)){
			...
			} else $this->msg=$this->t("Carrito de compras vacío");
		} else $this->msg=$this->t("Carrito de compras vacío");
				   ...
				   if (AddonMobileApp::isArray($json_data)) {
					   ...
					   if ($this->data['payment_list']=="cod" || 
					      ...
					      $this->data['payment_list']=="obd" ){				      	
		    				...
		    				} else $status="En preparación Artesano de la Comida";
					   ...
					   $params=array(
					    ...
					    'stats_id'=>isset($this->data['stats_id'])?$this->data['stats_id']:0,
					    'date_created'=>date('Y-m-d H:i:s'),
					    'ip_address'=>$_SERVER['REMOTE_ADDR'],
					    'order_change'=>(isset($this->data['order_change']) && !empty($this->data['order_change']))?$this->data['order_change']: 0,
					    'mobile_cart_details'=>(isset($this->data['cart']) && !empty($this->data['cart']))?$this->data['cart']:'',
					   );
		...
		if ( $Validator->validate()){
		   if ( $res=yii::app()->functions->isClientExist($this->data['email_address']) ){					
			...
			if ($DbExt->updateData("{{client}}",$params,'client_id',$res['client_id'])){
                ...
                if (!sendEmail($to,$sender,Yii::t("default","Recuperar contraseña Food Market"),$tpl)){		    			                	
                	$this->details="failed";
