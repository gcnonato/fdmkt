...
$merchant_food_handling_card_photo=Yii::app()->functions->getOption("merchant_food_handling_card_photo",$merchant_id);
$merchant_food_message=Yii::app()->functions->getOption("merchant_food_message",$merchant_id);
$merchant_food_choice=Yii::app()->functions->getOption("merchant_food_choice",$merchant_id);
...
if($merchant_food_handling_card_photo)
  $display="block";
else
  $display="none";

if($merchant_food_message)
  $displayMsg="none";
else
  $displayMsg="none";
...
<h3  style="display:none"><?php echo Yii::t("default","Tips")?></h3>
...
<div class="uk-form-row">
 <div class="uk-form-label">Carnet de manipulación de alimentos</div>
...
<div id="food_handling_card_pic_row">
<div class="uk-form-row"> 
 <label class="uk-form-label">  </label>
  <div style="display:inline-table;margin-left:1px;" class="button uk-button" id="photo3"><?php echo Yii::t('default',"Browse")?></div>	  
  <DIV  style="display:none;" class="photo2_chart_status" >
	<div id="percent_bar" class="photo2_percent_bar"></div>
	<div id="progress_bar" class="photo2_progress_bar">
	  <div id="status_bar" class="photo2_status_bar"></div>
...
<?php if (!empty($merchant_food_handling_card_photo)):?>
<div class="uk-form-row"> 
<?php else :?>
<div class="input_block preview">
<?php endif;?>
...
<div class="image_preview3">
 <?php if (!empty($merchant_food_handling_card_photo)):?>
 <input type="hidden" name="photo3" value="<?php echo $merchant_food_handling_card_photo;?>">
 <img class="uk-thumbnail uk-thumbnail-small" src="<?php echo Yii::app()->request->baseUrl."/upload/".$merchant_food_handling_card_photo;?>?>" alt="" title="">

 <div>
   <a href="javascript:;" class="remove-food-image"><?php echo Yii::t("default","Remove")?></a>
 </div>
 <?php endif;?>
</div>
</div>
...
<div class="uk-form-row" id="m_food_message" style="display:none !important">
 <!-- <label class="uk-form-label">Food Message</label> -->
   <label class="uk-form-label">Permiso de manipulación de alimentos</label>
  <?php 
  echo CHtml::textArea('merchant_food_message',
  Yii::app()->functions->getOption("merchant_food_message",$merchant_id)
  ,array(
  'class'=>"uk-form-width-large"
  ));
  ?>
...
$(document).ready(function(){
   var status1="<?php echo $display?>";
   var status2="<?php echo $displayMsg?>";
   var v ="<?php echo $merchant_food_choice?>";
   
   var options1 = "";
   var options = "";
   var data = "";  
   var i = ["Selecciona una opción","Al día","En proceso","No cuento con permiso"];
   options1 = "<select id='food_handling_card' required name='food_handling_card' class='grey-fields full-width'>";
   for(var a = 0; a < i.length; a++){
     if(i[a]==v)
      options = "<option selected vlaue='"+i[a]+"' > " + i[a] + "</option>";
      else
      options = "<option vlaue='"+i[a]+"' > " + i[a] + "</option>";
      data = data + options;
   };
   $('#ddlCats').append(options1+data+"</select>");
   $("#food_handling_card_pic_row").css("display",status1);
   $("#m_food_message").css('display',status2);
   ...
   var checkTxt=$("#food_handling_card" ).val();
     if(checkTxt!='')
     {
       if(checkTxt=='Al día')
       {
         ...
         $( "#m_food_message" ).css("display","none");
       }
       else
       {
        if(checkTxt=='')
