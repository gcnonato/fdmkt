...
$action="<a href=\"$link\" >".Yii::t("default","Details")."</a>";
	...
	{		
		...
		$token=md5($this->data['restaurant_name'].date('Y-m-d H:i:s'));
	    ...
	    $params=array(
		  ...
		  'food_handling_card' => $this->data['food_handling_card'],
		  'food_handling_card_photo' => $this->data['photo'],  
	    ...
	    //echo "==>".$this->data['food_handling_card_pic'];
	    if ( !Yii::app()->functions->validateUsername($this->data['username']) ){
		    ...
		    if ($this->insertData("{{merchant}}",$params)){
				...
				sendEmail("andres@buzz.cr",$sender,"Nuevo Puesto de Comidas",$tpl);	    		  
	            if (!sendEmail($to,$sender,"Bienvenid@ a Food Market",$tpl)){		    	
					//$this->details="failed";
		...
		if ( $this->code==1){
			...
			if (!empty($tpl)){
                ...
                if (empty($wd_template_payout_subject)){
	                $wd_template_payout_subject=t("Tu solicitud de retiro ha sido recibida");
                }                
...
$this->msg=t("We have sent bank information instruction to your email")." :$merchant_email";
	...
	{		
		if( $res=Yii::app()->functions->getClientInfo($this->data['client_id'])){
			if ( $this->data['code']==$res['mobile_verification_code']){
				...
				$params=array( 
				  'status'=>"active",
				  'mobile_verification_date'=>date('Y-m-d H:i:s')
				);
	...
	public function sendUpdateOrderEmail()
	{
		...
		if ($res=Yii::app()->functions->getOrder($this->data['order_id'])){
			...
			$subject_title='=?UTF-8?B?'.base64_encode($this->data['subject']).'?=';
