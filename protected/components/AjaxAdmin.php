...
if (!class_exists('AjaxAdmin'))
{
	class AjaxAdmin extends DbExt
	{
		...
		public function addMerchant()
		{
			...
			if(isset($this->data['membership_expired']) && !empty($this->data['membership_expired'])){
				$params['membership_expired']=$this->data['membership_expired'];
			}else{
				$params['membership_expired']=null;
			}
			
			$params['is_featured']=isset($this->data['is_featured'])?$this->data['is_featured']:1;
			
			if(isset($this->data['package_id']) && !empty($this->data['package_id'])){
			...
			if (empty($this->data['id'])){
				if ( $this->insertData("{{merchant}}",$params)){
					...
					if($this->data['merchant_food_handling_card_photo']!='')
					{
					Yii::app()->functions->updateOption("merchant_food_handling_card_photo",
                        isset($this->data['merchant_food_handling_card_photo'])?$this->data['merchant_food_handling_card_photo']:''
                        ,$mtid);

					Yii::app()->functions->updateOption("merchant_food_message",
                        isset($this->data['merchant_food_message'])?'':''
                        ,$mtid);	
					}

					if($this->data['merchant_food_message']!='')
					{
					Yii::app()->functions->updateOption("merchant_food_message",
                        isset($this->data['merchant_food_message'])?$this->data['merchant_food_message']:''
                        ,$mtid);
					
					Yii::app()->functions->updateOption("merchant_food_handling_card_photo",
                        isset($this->data['merchant_food_handling_card_photo'])?'':''
                        ,$mtid);
					}
				}
			} else {
				unset($params['date_created']);
				$params['date_modified']=date('Y-m-d');
				...
				if ($res){
					...
					Yii::app()->functions->updateOption("merchant_longtitude",
                        isset($this->data['merchant_longtitude'])?$this->data['merchant_longtitude']:'byyyy'
                        ,$this->data['id']);
 
                    Yii::app()->functions->updateOption("merchant_food_message",
                        isset($this->data['merchant_food_message'])?$this->data['merchant_food_message']:'hello'
                        ,$this->data['id']);
		...
		public function rowDelete()
		{
			...
			if ($this->data['tbl']=="merchantSponsoredList"){
				$params=array('is_sponsored'=>1,'date_modified'=>date('Y-m-d'));
		...
		public function merchantSettings()
		{
			...
			$this->data['merchant_timezone'] = "America/Costa_Rica";
			...
			if($this->data['food_handling_card']=='Al día')
			{	
			Yii::app()->functions->updateOption("merchant_food_handling_card_photo",
                isset($this->data['photo3'])?$this->data['photo3']:''
                ,$merchant_id);
			Yii::app()->functions->updateOption("merchant_food_message",
                isset($this->data['merchant_food_message'])?'':''
                ,$merchant_id);
			...
			Yii::app()->functions->updateOption("merchant_food_choice",
                isset($this->data['food_handling_card'])?$this->data['food_handling_card']:''
                ,$merchant_id);
			...
			$connection=Yii::app()->db;
			$sql = "UPDATE ka_merchant SET food_handling_card = '".$this->data['food_handling_card']."' WHERE merchant_id = $merchant_id";
				...
				if ( Yii::app()->functions->code==1){
					...
					if (is_array($raw) && count($raw)>=1){
						$params=array(
							...
							'cc_id'=>isset($this->data['cc_id'])?intval($this->data['cc_id']): 0,
						...
						/** add tips */
						$params['cart_tip_percentage']=isset($this->data['cart_tip_percentage'])?floatval($this->data['cart_tip_percentage']):0;
						$params['cart_tip_value']=isset($this->data['cart_tip_value'])?floatval($this->data['cart_tip_value']):0;
						/*if (isset($this->data['cart_tip_value'])){
                        	if (is_numeric($this->data['cart_tip_value'])){
                        		$params['total_w_tax']=$params['total_w_tax']+$this->data['cart_tip_value'];total_commission
                        	}
                        }*/
		...
		public function addReviews()
		{
			...
			if ( Yii::app()->functions->isClientLogin()){
				...
				if($this->data['initial_review_rating']<=2)
				{
					$r_to="andres@buzz.cr";
					$r_from="andres@buzz.cr";
					//$r_subject='=?UTF-8?B?'.base64_encode("Nuevo depósito bancario").'?=';
					$r_subject="Restaurante mal calificado";
					$r_tpl="Hay un restaurante que ha caído en malas calificaciones. Contactar e identificar lo ocurrido";
					if ( Yii::app()->functions->sendEmail($r_to,$r_from,$r_subject,$r_tpl) ){
						$this->code=1;
		...
		public function removeFoodimage()
		{
			if (Yii::app()->functions->isMerchantLogin()){
				$DbExt=new DbExt;
				$merchant_id=Yii::app()->functions->getMerchantID();
		    	...
		    	{{option}}
		    	WHERE
		    	option_name='merchant_food_handling_card_photo'
		    	AND
		    	merchant_id='$merchant_id'
		    	";
				$DbExt->qry($stmt);
		...
		public function updateOrder()
		{
			...
			if (isset($this->data['order_id'])){
				...
				if ( $resp=Yii::app()->functions->verifyOrderIdByOwner($order_id,$merchant_id) ){
					$params=array('status'=>$this->data['status'],'date_modified'=>date('Y-m-d'),'viewed'=>2);
				...
				switch ($this->data['rpt']) {
					case 'sales-report':
						...
						$header=array(
							...
							Yii::t("default","Date"));
						$filename = $this->data['rpt'].'-'. date('Y-m-d') .'.csv';
		...
		public function merchantSignUp()
		{
			...
			$token=md5($this->data['restaurant_name'].date('Y-m-d'));
			$params=array(
			...
			if ( !Yii::app()->functions->validateUsername($this->data['username']) ){
				...
				if ($this->insertData("{{merchant}}",$params)){
					...
					if (!sendEmail($to,$sender,"¡Bienvenido a Food Market!",$tpl)){
						//$this->details="failed";
				...
				if ($res=$this->rst($stmt)){
					if ($res[0]['status']=="active"){
						...
						if ($res[0]['activation_key']==$this->data['activation_code']){
							...
							$params=array('status'=>"active",'date_activated'=>date('Y-m-d'),'ip_address'=>$_SERVER['REMOTE_ADDR']);
					...
					if (is_numeric($this->data['cc_id'])){
						...
						if (isset($this->data['renew'])){
							...
							$params_update=array(
								...
								'membership_expired'=>$membership_info['membership_expired'],
								'membership_purchase_date'=>date('Y-m-d'),
								'status'=>'active'
							);
		...
		public function contacUsSubmit()
		{
			...
			if ($validator->validate()){
				...
				$subject=Yii::t("default","Nuevo contacto en FoodMarket");
			...
			if ($Validator->validate()){
				if ( $res=yii::app()->functions->isClientExist($this->data['username-email']) ){
					...
					if ($this->updateData("{{client}}",$params,'client_id',$res['client_id'])){
						...
						if (!sendEmail($to,$sender,Yii::t("default","¿Necesitás recuperar tu contraseña?"),$tpl)){
							$this->details="failed";
			...
			if ($Validator->validate()){
				if ( $res=Yii::app()->functions->getLostPassToken($this->data['token'])){
					$params=array(
						...
						'date_modified'=>date('Y-m-d'),
						'ip_address'=>$_SERVER['REMOTE_ADDR'],
						'lost_password_token'=>md5(date('Y-m-d'))
					);
					if ( $this->updateData("{{client}}",$params,'client_id',$res['client_id'])){
						$this->code=1;
			...
			if ( isset($this->data['email_address'])){
				if ($res=yii::app()->functions->isMerchantExist($this->data['email_address']) ){
					...
					if ( $this->updateData("{{merchant}}",$params,'merchant_id',$res[0]['merchant_id'])){
						...
						if (!sendEmail($to,$sender,t("¿Necesitás recuperar tu contraseña?"),$tpl)){
							$this->details="failed";
						} else $this->details="ok mail";
		...
		public function resendActivationCode()
		{
			...
			if ($res=$this->rst($stmt)){
				...
				$sender=Yii::app()->functions->getOptionAdmin('website_contact_email');
				$to=$res[0]['contact_email'];
				if (!sendEmail($to,$sender,Yii::t("default","Bienvenid@ a Food Market"),$tpl)){
					$this->details="failed";
				} else $this->details="ok mail";
				$this->code=1;
		...
		public function addLanguage()
		{
			...
			if ($Validator->validate()){
				...
				if (is_numeric($this->data['id'])){
					unset($params['date_created']);
					$params['last_updated']=date('Y-m-d');
		...
		public function bankDepositVerification()
		{
			...
			if ($res=Yii::app()->functions->getMerchantByToken($this->data['ref'])){
				...
				if ($DbExt->insertData("{{bank_deposit}}",$params)){
					...
					$from='no-reply@'.$_SERVER['HTTP_HOST'];
					$subject=Yii::t("default","Nuevo depósito bancario");
			...
			if ( isset($this->data['email_address'])){
				if ($res=yii::app()->functions->isAdminExist($this->data['email_address']) ){
					...
					if ( $this->updateData("{{admin_user}}",$params,'admin_id',$res[0]['admin_id'])){
						...
						if (!sendEmail($to,$sender,t("Admin: ¿Olvidaste tu contraseña?"),$tpl)){
