	...
	class AjaxAdmin extends DbExt
	{
		...
		public function addReviews()
		{
			...
			if($this->data['initial_review_rating']<=2)
			{
				...
				$r_subject='=?UTF-8?B?'.base64_encode("Restaurante mal calificado").'?=';
				$r_body='=?UTF-8?B?'.base64_encode("Restaurante mal calificado").'?=';
		...
		public function contacUsSubmit()
		{
			...
			if ($validator->validate()){
				...
				$subject_title='=?UTF-8?B?'.base64_encode("Nuevo contacto en FoodMarket").'?=';
				$subject=Yii::t("default",$subject_title);
		...
		public function bookATableMerchant()
		{
			...
			if (isset($this->data['send_email'])){
				if ($this->code==1 || $this->data['send_email']==1){
					...
					// Below one line is added by Harish for allowing "Enable special characters on all email subject. Example: é, ñ"
					$subject_title='=?UTF-8?B?'.base64_encode($subject).'?=';
					$subject=empty($subject)?"Booking Table":$subject_title;		
		...
		public function bankDepositVerification()
		{
			...
			if ($res=Yii::app()->functions->getMerchantByToken($this->data['ref'])){
				...
				if ($DbExt->insertData("{{bank_deposit}}",$params)){
					...
					$from='no-reply@'.$_SERVER['HTTP_HOST'];
					$subject_title='=?UTF-8?B?'.base64_encode("Nuevo depósito bancario").'?=';
					$subject=Yii::t("default",$subject_title);
